...
# The controlled replay is intentionally self-contained. These definitions
# replace dependencies on the training experiment's configuration and state.
CONDITION_COLUMNS = [
    "experiment_id",
    "run_id",
    "operator",
...
def yyyymmddhhmmss_log_path(log_dir: str, suffix: str = ".csv") -> Path:
    ...
    for index in range(1, 1000):
        candidate = directory / f"{base}_{index:03d}{suffix}"
        if not candidate.exists():
            return candidate
