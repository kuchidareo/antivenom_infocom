...
def _should_match_reference_size(dataset_name: str) -> bool:
    return dataset_name != REFERENCE_DATASET_NAME and dataset_name in MATCH_REFERENCE_SIZE_DATASETS


def _reference_clean_split_counts(data_dir: str, reference_dataset_name: str = REFERENCE_DATASET_NAME) -> Dict[str, int]:
    ...
    return {split: len(ds) for split, ds in _iter_splits(reference)}
...
def _maybe_match_reference_split_size(
    ...
    *,
    ...
    if split not in reference_counts:
        return ds.select([]), []

    target_count = min(reference_counts[split], len(ds))
