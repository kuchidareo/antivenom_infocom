...
raw or per-instruction mode requested on the command line.
"""
...
DEFAULT_OUTPUT = DEFAULT_COLLECTION / "epochwise_clean_baseline_comparison_per_instruction"
DEFAULT_RAW_OUTPUT = DEFAULT_COLLECTION / "epochwise_clean_baseline_comparison_raw"
...
def estimate_profiles(args: argparse.Namespace, conditions: set[str]) -> None:
    ...
    print(f"Estimating requested {args.counter_normalization} profiles...", flush=True)
...
def resolve_profile_dir(
    ...
    if normalization == "raw":
        candidates = [collection_dir / "distribution_profile_cache"]
    else:
        candidates = sorted(collection_dir.glob("distribution_profile_cache_per_instruction*"))
    compatible = [
