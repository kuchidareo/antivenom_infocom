...
CIFAR10_DATASET_NAME="uoft-cs/cifar10"
TRASHNET_DATASET_NAME="kuchidareo/small_trashnet"
CLIENT_ID="client_1"
REFERENCE_TRIALS=0
NUM_ROUNDS=10
NONIID_ALPHA=0.3
...
# label|dataset_name|data_dir|poisoning_method|augmentation_profile|model|partition_method|model_depth
STAGE_SPECS=(
  "cifar10_iid|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|baseline|simple_cnn|iid|3"
  "cifar10_non_iid|${CIFAR10_DATASET_NAME}|${REMOTE_NONIID_DATA_DIR}|clean|baseline|simple_cnn|dirichlet_noniid|3"
  "cifar10_moderate_augmentation|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|moderate|simple_cnn|iid|3"
  "cifar10_strong_augmentation|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|strong|simple_cnn|iid|3"
  "cifar10_availability_shortcut|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|availability_shortcuts|baseline|simple_cnn|iid|3"
  "cifar10_vit|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|baseline|tiny_vit|iid|4"
  "cifar10_vit_non_iid|${CIFAR10_DATASET_NAME}|${REMOTE_NONIID_DATA_DIR}|clean|baseline|tiny_vit|dirichlet_noniid|4"
  "cifar10_vit_moderate_augmentation|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|moderate|tiny_vit|iid|4"
  "cifar10_vit_strong_augmentation|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|strong|tiny_vit|iid|4"
  "cifar10_vit_availability_shortcut|${CIFAR10_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|availability_shortcuts|baseline|tiny_vit|iid|4"
  "trashnet_iid|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|baseline|simple_cnn|iid|3"
  "trashnet_non_iid|${TRASHNET_DATASET_NAME}|${REMOTE_NONIID_DATA_DIR}|clean|baseline|simple_cnn|dirichlet_noniid|3"
  "trashnet_moderate_augmentation|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|moderate|simple_cnn|iid|3"
  "trashnet_strong_augmentation|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|strong|simple_cnn|iid|3"
  "trashnet_vit|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|baseline|tiny_vit|iid|4"
  "trashnet_vit_non_iid|${TRASHNET_DATASET_NAME}|${REMOTE_NONIID_DATA_DIR}|clean|baseline|tiny_vit|dirichlet_noniid|4"
  "trashnet_vit_moderate_augmentation|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|moderate|tiny_vit|iid|4"
  "trashnet_vit_strong_augmentation|${TRASHNET_DATASET_NAME}|${REMOTE_IID_DATA_DIR}|clean|strong|tiny_vit|iid|4"
)
...
Default `both` flow:
  git pull -> sync IID/non-IID CIFAR-10 and Small TrashNet -> check -> run -> collect

The 18 sequential CPU-only stages on 192.168.0.141 are:
  CIFAR-10, SimpleCNN and TinyViT:
    IID clean, Dirichlet non-IID, moderate augmentation,
    strong augmentation, and availability shortcut
  Small TrashNet, SimpleCNN and TinyViT:
    IID clean, Dirichlet non-IID, moderate augmentation,
    and strong augmentation

Small TrashNet availability-shortcut stages are intentionally omitted.
...
sync_dataset_root() {
  ...
  local dataset_slug="$3"
  local source_dir="${local_data_dir}/${dataset_slug}"
  local destination_dir="${remote_data_dir}/${dataset_slug}"
  local -a rsync_command

  test -f "${source_dir}/partition_metadata.csv" || {
    print "Missing local dataset metadata: ${source_dir}/partition_metadata.csv" >&2
    return 2
  }
...
sync_datasets() {
  local dataset_slug
  for dataset_slug in cifar10 small_trashnet; do
    sync_dataset_root "$LOCAL_IID_DATA_DIR" "$REMOTE_IID_DATA_DIR" "$dataset_slug"
    sync_dataset_root "$LOCAL_NONIID_DATA_DIR" "$REMOTE_NONIID_DATA_DIR" "$dataset_slug"
  done
}
...
check_environment() {
  ...
  ssh_run "
    ...
    test -f '$REMOTE_IID_DATA_DIR/small_trashnet/partition_metadata.csv'
    test -f '$REMOTE_NONIID_DATA_DIR/small_trashnet/partition_metadata.csv'
    test -f '$REMOTE_IID_DATA_DIR/small_trashnet/augmented/moderate/PREPARED'
    test -f '$REMOTE_IID_DATA_DIR/small_trashnet/augmented/strong/PREPARED'
...
run_stage() {
  ...
  local dataset_name="${fields[2]}"
  local data_dir="${fields[3]}"
  local poisoning_method="${fields[4]}"
  local augmentation_profile="${fields[5]}"
  local model_name="${fields[6]}"
  local partition_method="${fields[7]}"
  local model_depth="${fields[8]}"
  ...
  print "==> stage ${label}"
  print "    dataset=${dataset_name} data=${data_dir}"
...
run_experiments() {
  print "Running ${#STAGE_SPECS} CIFAR-10/Small-TrashNet stages sequentially on ${JETSON_HOST} CPU"
