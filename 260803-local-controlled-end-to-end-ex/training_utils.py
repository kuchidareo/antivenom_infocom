    ...
    *,
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            if layer_perf_logger is not None:
                layer_perf_logger.begin_batch(epoch=epoch, batch_idx=batch_idx, round_id=round_id)
            try:
                ...
                if layer_perf_logger is not None:
                    layer_perf_logger.abort_batch()
                raise
            if layer_perf_logger is not None:
                layer_perf_logger.flush_batch()
...
def replay_model_with_frozen_state(
    *,
    model: torch.nn.Module,
    data_loader: Any,
    epochs: int,
    ...
    """Run real-data forward/backward passes without changing model state."""
    device = get_device()
    model.to(device)
    for parameter in model.parameters():
        parameter.requires_grad_(True)

    # Keep Dropout active as in training, but prevent BatchNorm running-stat updates.
    model.train()
    ...
    for epoch in range(epochs):
        ...
        while True:
            ...
            if metrics_logger is not None:
                metrics_logger.write(
                    state=state.snapshot(),
                    metric_event="frozen_replay_batch",
                    metric_split="frozen_replay",
                    loss=loss_value,
                    accuracy=batch_correct / max(batch_size, 1),
                    num_examples=float(batch_size),
                    batch_size_actual=batch_size,
        ...
        if metrics_logger is not None:
            metrics_logger.write(
                state=state.snapshot(),
                metric_event="frozen_replay_epoch",
                metric_split="frozen_replay",
                loss=epoch_loss / max(epoch_seen, 1),
                accuracy=epoch_correct / max(epoch_seen, 1),
                num_examples=float(epoch_seen),
    ...
    changed = [
        name
        for name, value in model.state_dict().items()
        if not torch.equal(value, frozen_state[name])
    ]
    if changed:
        raise RuntimeError(
            "Frozen replay changed model state: " + ", ".join(changed)
        )
    ...
    if metrics_logger is not None:
        metrics_logger.write(
            state=state.snapshot(),
            metric_event="frozen_replay_summary",
            metric_split="frozen_replay",
            loss=total_loss / max(total_seen, 1),
            accuracy=total_correct / max(total_seen, 1),
            num_examples=float(total_seen),
