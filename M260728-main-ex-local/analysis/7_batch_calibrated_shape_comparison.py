...
runs are paired by trial, epoch, and batch index. Trials are aggregated within
each device before statistical inference, so intervals, batches, and trials are
not treated as independent device replicates.
"""
...
sys.path.insert(0, str(PROFILE_TOOL_DIR))
...
DEFAULT_OUTPUT = SCRIPT_DIR / "batch_calibrated_shape_comparison"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--targets", nargs="+", choices=tuple(ROLE_LABELS), default=tuple(ROLE_LABELS)
    )
    ...
    if args.min_matched_batches < 2 or args.min_paired_devices < 2:
        parser.error("minimum batch/device counts must be at least 2")
    if args.trials_per_device < 1 or args.expected_devices < 1:
        parser.error("trial and expected-device counts must be positive")
...
def resolve_device_inputs(path: Path, requested_devices: list[str]) -> dict[str, Path]:
    ...
    for device_id in requested_devices:
        candidates = (root / device_id / Path(*STAGE_PARTS), root / device_id)
        selected = next(
            (
                candidate for candidate in candidates
                if candidate.is_dir() and any(candidate.rglob("*_perf.csv"))
            ),
            None,
        )
        if selected is None:
            warnings.warn(f"Skipping {device_id}: no phase1 CIFAR-10 perf logs")
            continue
        resolved[device_id] = selected
    if not resolved:
        raise FileNotFoundError(f"No requested device logs below {root}")
    return resolved
...
def main() -> int:
    args = parse_args()
    device_inputs = resolve_device_inputs(args.input_dir, list(args.devices))
    single_device_mode = len(device_inputs) == 1
    if len(device_inputs) != args.expected_devices:
        warnings.warn(
            f"Discovered {len(device_inputs)} of {args.expected_devices} expected devices"
        )
    ...
    indexed_by_device = {
        device_id: index_runs(discover_runs(input_dir))
        for device_id, input_dir in device_inputs.items()
    }
    ...
    requested_groups: list[
        tuple[str, str, str, str, str, str, list[str], dict[tuple[str, str], RunFiles]]
    ] = []
    for device_id, indexed in indexed_by_device.items():
        for dataset in args.datasets:
            for model in args.models:
                clean_condition = condition_name(dataset, model, "clean")
                for target_role in args.targets:
                    target_condition = condition_name(dataset, model, target_role)
                    clean_trials = {
                        trial for condition, trial in indexed
                        if condition == clean_condition and trial in valid_trials
                    }
                    target_trials = {
                        trial for condition, trial in indexed
                        if condition == target_condition and trial in valid_trials
                    }
                    trials = sorted(clean_trials & target_trials)
                    if not trials:
                        warnings.warn(
                            f"Skipping {device_id}/{dataset}/{model}/{target_role}: "
                            f"no paired {clean_condition}/{target_condition} runs"
                        )
                        continue
                    requested_groups.append(
                        (
                            device_id, dataset, model, target_role, clean_condition,
                            target_condition, trials, indexed,
                        )
                    )

    total_fits = sum(
        len(trials) * len(args.epochs) * len(args.counters)
        for *_, trials, _ in requested_groups
    )
    fit_index = 0
    for (
        device_id, dataset, model, target_role, clean_condition,
        ...
        for trial_id in trials:
            try:
                ...
                skipped.append(f"{device_id}/{dataset}/{model}/{target_role}/{trial_id}: {exc}")
                continue
                ...
                for counter in args.counters:
                    ...
                    if fit_index == 1 or fit_index % 100 == 0 or fit_index == total_fits:
                        print(
                            ...
                            f"{device_id}/{target_role}/{trial_id}/epoch{epoch}/{counter}",
                            flush=True,
                        )
                    ...
                    if len(matched) < args.min_matched_batches:
                        skipped.append(
                            f"{device_id}/{dataset}/{model}/{target_role}/{trial_id}/"
                            f"epoch{epoch}/{counter}: "
                    ...
                    if not clean_fit.success or not target_fit.success:
                        skipped.append(
                            f"{device_id}/{dataset}/{model}/{target_role}/{trial_id}/"
                            f"epoch{epoch}/{counter}: optimizer failure"
                        )
                        continue
    ...
    for key, group in grouped:
        ...
        clean = group[group["role"].eq("clean")].pivot(
            index=["device_id", "trial_id"], columns="bin_index", values="shape_density"
        )
        target = group[group["role"].eq("target")].pivot(
            index=["device_id", "trial_id"], columns="bin_index", values="shape_density"
        )
        paired_trials = clean.index.intersection(target.index)
        clean = clean.loc[paired_trials].sort_index(axis=1)
        target = target.loc[paired_trials].sort_index(axis=1)
        trial_counts = pd.Series(
            [index[0] for index in paired_trials]
        ).value_counts()
        complete_devices = sorted(
            trial_counts[trial_counts.eq(args.trials_per_device)].index.astype(str)
        )
        required_devices = 1 if single_device_mode else args.min_paired_devices
        if len(complete_devices) < required_devices:
            skipped.append(
                f"{dataset}/{model}/{target_role}/epoch{epoch}/{counter}: "
                f"{len(complete_devices)} paired devices"
            )
            continue
        clean = clean.loc[clean.index.get_level_values("device_id").isin(complete_devices)]
        target = target.loc[target.index.get_level_values("device_id").isin(complete_devices)]
        if single_device_mode:
            # Preserve the original 7_ behavior for a one-device exploratory run:
            # the three paired trials are the inference units. With only three
            # pairs, p-values are coarse and cannot establish 5% significance.
            clean_array = clean.to_numpy(dtype=float)
            target_array = target.reindex(clean.index).to_numpy(dtype=float)
            inference_unit = "paired_trial"
            paired_units = len(clean_array)
        else:
            clean_device = clean.groupby(level="device_id").mean().reindex(complete_devices)
            target_device = target.groupby(level="device_id").mean().reindex(complete_devices)
            clean_array = clean_device.to_numpy(dtype=float)
            target_array = target_device.to_numpy(dtype=float)
            inference_unit = "paired_device"
            paired_units = len(complete_devices)
        ...
        efficiency = group.drop_duplicates(["device_id", "trial_id", "role"])
        if single_device_mode:
            clean_efficiency = (
                efficiency[efficiency["role"].eq("clean")]
                .set_index(["device_id", "trial_id"])["total_efficiency"]
                .reindex(clean.index).to_numpy(dtype=float)
            )
            target_efficiency = (
                efficiency[efficiency["role"].eq("target")]
                .set_index(["device_id", "trial_id"])["total_efficiency"]
                .reindex(clean.index).to_numpy(dtype=float)
            )
        else:
            clean_efficiency = (
                efficiency[efficiency["role"].eq("clean")]
                .groupby("device_id")["total_efficiency"].mean()
                .reindex(complete_devices).to_numpy(dtype=float)
            )
            target_efficiency = (
                efficiency[efficiency["role"].eq("target")]
                .groupby("device_id")["total_efficiency"].mean()
        ...
        test_rows.append({
            ...
            "counter": counter,
            "epoch": int(epoch),
            "paired_devices": len(complete_devices),
    ...
    for _, group in tests.groupby(
        ["dataset", "model", "target_role", "counter"], sort=False
    ):
        group = group.copy()
    ...
    if not args.no_plots:
        figure_groups = tests[["dataset", "model", "target_role"]].drop_duplicates()
        for figure_group in figure_groups.itertuples(index=False):
            dataset, model, target_role = (
                figure_group.dataset, figure_group.model, figure_group.target_role
            )
            destination = output_dir / dataset / model / target_role
            target_shape_summary = shape_summary[
                shape_summary["target_role"].eq(target_role)
            ]
            target_pointwise = pointwise[pointwise["target_role"].eq(target_role)]
            target_total_summary = total_summary[
                total_summary["target_role"].eq(target_role)
            ]
            target_tests = tests[tests["target_role"].eq(target_role)]
            for counter in args.counters:
                selected_tests = target_tests[
                    target_tests["dataset"].eq(dataset)
                    & target_tests["model"].eq(model)
                    & target_tests["counter"].eq(counter)
                ]
                if selected_tests.empty:
                    continue
                plot_epochs = sorted(selected_tests["epoch"].astype(int).unique())
                plot_shape_epochs(
                    target_shape_summary, target_tests, model=model, counter=counter,
                    epochs=plot_epochs, target_label=ROLE_LABELS[target_role],
                    output_dir=destination,
                    formats=list(args.formats), dpi=args.dpi,
                )
                plot_difference_epochs(
                    target_pointwise, target_tests, model=model, counter=counter,
                    epochs=plot_epochs, target_label=ROLE_LABELS[target_role],
                    output_dir=destination,
                    formats=list(args.formats), dpi=args.dpi,
                )
                plot_total_efficiency(
                    target_total_summary, target_tests, model=model, counter=counter,
                    target_label=ROLE_LABELS[target_role], output_dir=destination,
                    formats=list(args.formats), dpi=args.dpi,
                )
                figures += 3
    ...
    summary = {
        ...
        "statistical_unit": (
            "paired trial (single-device exploratory mode)"
            if single_device_mode else "paired device"
        ),
        "within_device_aggregation": (
            f"mean of {args.trials_per_device} paired trial profiles before inference"
        ),
        "multiple_testing": (
            "BH independently across requested epochs for each "
            "dataset/model/target/counter"
        ),
