...
class HardwareLogger:
    def __init__(
        ...
        *,
        ...
        fps: float = 10.0,
        sample_phases: Optional[Iterable[str]] = None,
    ) -> None:
        if fps <= 0:
        ...
        self.sample_phases: Optional[Set[str]] = (
            None if sample_phases is None else set(sample_phases)
        )
        unknown_phases = set() if self.sample_phases is None else self.sample_phases - PHASES
        if unknown_phases:
            raise ValueError(f"Invalid sample phases: {sorted(unknown_phases)}")
    ...
    def _should_write(self, row: Dict[str, Any]) -> bool:
        return self.sample_phases is None or row.get("phase") in self.sample_phases
    ...
    def _sample(self) -> Dict[str, Any]:
        ...
        row = {
            ...
            "system_cpu_freq_mhz": freq.current if freq is not None else "",
            "system_cpu_freq_core_0_mhz": per_core_freq[0].current if len(per_core_freq) > 0 else "",
            "system_cpu_freq_core_1_mhz": per_core_freq[1].current if len(per_core_freq) > 1 else "",
            "system_cpu_freq_core_2_mhz": per_core_freq[2].current if len(per_core_freq) > 2 else "",
            "system_cpu_freq_core_3_mhz": per_core_freq[3].current if len(per_core_freq) > 3 else "",
