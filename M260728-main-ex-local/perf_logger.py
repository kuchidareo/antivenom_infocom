...
from hardware_logger import PHASES, TrainingState
...
def perf_event_runtime_columns(events: Sequence[str]) -> List[str]:
    return [f"perf_{sanitize_event_name(event)}_runtime_ns" for event in events]
...
def parse_perf_stat_csv_line(line: str) -> Optional[Dict[str, Any]]:
    ...
    layout is:
        ...
        elapsed_sec,count,unit,event,runtime_ns,enabled_pct,...
    ...
    parsing for event runtime and enabled percentages.
    """
    stripped = line.strip()
    ...
    runtime_ns = parse_perf_number(parts[4]) if len(parts) > 4 else ""
...
class PerfLogger:
    ...
    def _run(self) -> None:
        with self.path.open("w", newline="") as f:
            ...
            for line in self._process.stderr:
                ...
                current_runtime[f"{event_column}_runtime_ns"] = parsed["runtime_ns"]
    ...
    def _write_sample(
        ...
        *,
        ...
        if self.sample_phases is not None and row.get("phase") not in self.sample_phases:
            return False
        writer.writerow(row)
        return True
