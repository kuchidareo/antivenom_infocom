...
def main() -> None:
    ...
    parser.add_argument("--model", default="tiny_vit")
    ...
    parser.add_argument("--vit-mlp-ratio", type=int, default=2)
    ...
    operators = [
        name for name, module in model.named_modules() if name and not any(module.children())
    ]
    expected_operators = 3 + 13 * args.model_depth
    if len(operators) != expected_operators:
        raise ValueError(
            f"Expected {expected_operators} TinyViT operators, got "
    ...
    print(f"operators={len(operators)} names={','.join(operators)}")
