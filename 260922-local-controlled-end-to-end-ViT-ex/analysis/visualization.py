...
Each batch-level value is the sum of all measured operator values for a single
phase. Ratios are calculated after summation, not averaged per operator.
...
def validate_layout(data: pd.DataFrame) -> None:
    ...
    batch_columns = [*run_columns, "phase", "epoch_display", "batch_idx"]
    counts = data.groupby(batch_columns, observed=True).size().rename("operators")
    for run_key, run_counts in counts.groupby(level=run_columns, observed=True):
        unique_counts = sorted(run_counts.unique().tolist())
        if len(unique_counts) != 1:
            expected = run_counts.mode().iloc[0]
            examples = run_counts[run_counts != expected].head(10).to_dict()
            raise ValueError(
                f"Operator count changes between batches for run={run_key}: "
                f"counts={unique_counts}, examples={examples}"
            )
        phase_counts = run_counts.groupby(level="phase").first().to_dict()
        if set(phase_counts) != {"forward", "backward"} or len(set(phase_counts.values())) != 1:
            raise ValueError(
                f"Forward/backward operator counts differ for run={run_key}: {phase_counts}"
            )

    duplicates = data.groupby([*batch_columns, "layer_index"], observed=True).size()
    duplicates = duplicates[duplicates != 1]
    if not duplicates.empty:
        raise ValueError(
            "An operator was measured more than once in a phase/batch: "
            f"{duplicates.head(10).to_dict()}"
        )
