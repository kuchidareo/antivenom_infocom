...
DEFAULT_MODEL = "tiny_vit"
DEFAULT_MODEL_DEPTH = 4
DEFAULT_MODEL_WIDTH_MULTIPLIER = 1.0
DEFAULT_MODEL_TARGET_PAM_MB = 0.0
DEFAULT_MODEL_ESTIMATED_PAM_MB = ""
DEFAULT_MODEL_PARAMETER_COUNT = ""
DEFAULT_MODEL_PAM_CALIBRATION_STEPS = 12
DEFAULT_VIT_PATCH_SIZE = 4
DEFAULT_VIT_EMBED_DIM = 128
DEFAULT_VIT_HEADS = 4
DEFAULT_VIT_MLP_RATIO = 2
DEFAULT_BATCH_SIZE = 128
DEFAULT_LOCAL_EPOCHS = 1
DEFAULT_NUM_ROUNDS = 10
DEFAULT_LOCAL_ML_EPOCHS = 15
DEFAULT_LOCAL_ML_GLOBAL_CLEAN_REFERENCE_TRIALS = 0
DEFAULT_LOCAL_ML_ANALYSIS_TRIALS = 1
...
DEFAULT_REMOTE_PROJECT_DIR = "/home/rasheed/kuchida/antivenom_infocom/260922-local-controlled-end-to-end-ViT-ex"
...
DEFAULT_BACKGROUND_WORKLOAD_GROUP = "none"
DEFAULT_BACKGROUND_WORKLOAD_PROFILE = "none"
...
def add_common_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--model-depth", type=int, default=DEFAULT_MODEL_DEPTH)
    parser.add_argument("--model-width-multiplier", type=float, default=DEFAULT_MODEL_WIDTH_MULTIPLIER)
    parser.add_argument(
        "--model-target-pam-mb",
        type=float,
        default=DEFAULT_MODEL_TARGET_PAM_MB,
        help="For pam_cnn/adaptive_cnn, calibrate width to this theoretical activation PAM in MB. 0 disables calibration.",
    )
    parser.add_argument("--model-pam-calibration-steps", type=int, default=DEFAULT_MODEL_PAM_CALIBRATION_STEPS)
    parser.add_argument("--vit-patch-size", type=int, default=DEFAULT_VIT_PATCH_SIZE)
    parser.add_argument("--vit-embed-dim", type=int, default=DEFAULT_VIT_EMBED_DIM)
    parser.add_argument("--vit-heads", type=int, default=DEFAULT_VIT_HEADS)
    parser.add_argument("--vit-mlp-ratio", type=int, default=DEFAULT_VIT_MLP_RATIO)
    parser.add_argument("--batch-size", type=int, default=DEFAULT_BATCH_SIZE)
    parser.add_argument(
        "--max-train-samples",
        type=int,
        default=DEFAULT_MAX_TRAIN_SAMPLES,
        help="Deterministically limit each local training run; 0 uses the full partition.",
    )
    ...
    parser.add_argument(
        "--experiment-mode",
        choices=("train", "frozen_replay"),
        default="train",
    )
    parser.add_argument("--checkpoint-path", default="")
    parser.add_argument("--run-role", default="")
    parser.add_argument("--badsampler-kappa", type=float, default=DEFAULT_BADSAMPLER_KAPPA)
    parser.add_argument(
        "--background-workload-enabled",
        action="store_true",
        default=DEFAULT_BACKGROUND_WORKLOAD_ENABLED,
    )
...
def condition_columns(
    *,
    ...
    return {
        ...
        "experiment_mode": getattr(args, "experiment_mode", "train"),
        "checkpoint_path": getattr(args, "checkpoint_path", ""),
        "model_state_frozen": getattr(args, "experiment_mode", "train") == "frozen_replay",
        "optimizer_enabled": getattr(args, "experiment_mode", "train") == "train",
        "run_type": run_type,
        ...
        "model_depth": getattr(args, "model_depth", DEFAULT_MODEL_DEPTH),
        "model_width_multiplier": getattr(args, "resolved_model_width_multiplier", getattr(args, "model_width_multiplier", DEFAULT_MODEL_WIDTH_MULTIPLIER)),
        "model_target_pam_mb": getattr(args, "model_target_pam_mb", DEFAULT_MODEL_TARGET_PAM_MB),
        "model_estimated_pam_mb": getattr(args, "resolved_model_estimated_pam_mb", DEFAULT_MODEL_ESTIMATED_PAM_MB),
        "model_parameter_count": getattr(args, "resolved_model_parameter_count", DEFAULT_MODEL_PARAMETER_COUNT),
        "vit_patch_size": getattr(args, "vit_patch_size", DEFAULT_VIT_PATCH_SIZE),
        "vit_embed_dim": getattr(args, "vit_embed_dim", DEFAULT_VIT_EMBED_DIM),
        "vit_heads": getattr(args, "vit_heads", DEFAULT_VIT_HEADS),
        "vit_mlp_ratio": getattr(args, "vit_mlp_ratio", DEFAULT_VIT_MLP_RATIO),
        "batch_size": getattr(args, "batch_size", DEFAULT_BATCH_SIZE),
        "max_train_samples": getattr(args, "max_train_samples", DEFAULT_MAX_TRAIN_SAMPLES),
        ...
        "badsampler_replacement": poisoning_method == POISONING_METHOD_BADSAMPLING,
        "background_workload_enabled": bool(
            getattr(args, "background_workload_enabled", DEFAULT_BACKGROUND_WORKLOAD_ENABLED)
        ),
        "background_workload_group": getattr(
            args, "background_workload_group", DEFAULT_BACKGROUND_WORKLOAD_GROUP
        ),
        "background_workload_profile": getattr(
            args, "background_workload_profile", DEFAULT_BACKGROUND_WORKLOAD_PROFILE
        ),
    }
