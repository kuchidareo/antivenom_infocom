...
class QueryKeyMatMul(nn.Module):
    """Attention score matrix multiplication, exposed as a PMU operator."""
...
class AttentionScale(nn.Module):
    """Apply the fixed inverse-square-root attention scale."""
...
class AttentionValueMatMul(nn.Module):
    """Weighted value matrix multiplication, exposed as a PMU operator."""

    def forward(self, weights: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        return weights @ value


class ResidualAdd(nn.Module):
    """Residual addition kept explicit so forward and backward are measurable."""
...
class Attention(nn.Module):
    def __init__(self, dim: int, heads: int) -> None:
        ...
        self.qk_matmul = QueryKeyMatMul()
        self.scale_scores = AttentionScale(self.scale)
        self.softmax = nn.Softmax(dim=-1)
        self.av_matmul = AttentionValueMatMul()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scores = self.scale_scores(scores)
        weights = self.softmax(scores)
        attended = self.av_matmul(weights, value).transpose(1, 2).reshape(
            batch, tokens, dim
        )
...
class TransformerBlock(nn.Module):
    def __init__(self, dim: int, heads: int, mlp_ratio: int) -> None:
        ...
        self.attention_residual = ResidualAdd()
        self.norm2 = nn.LayerNorm(dim)
        self.fc1 = nn.Linear(dim, hidden)
        self.gelu = nn.GELU()
        self.fc2 = nn.Linear(hidden, dim)
        self.mlp_residual = ResidualAdd()

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.attention_residual(x, self.attn(self.norm1(x)))
        return self.mlp_residual(x, self.fc2(self.gelu(self.fc1(self.norm2(x)))))
...
class TinyViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Image inputs do not require gradients. The PMU logger therefore wraps
        # this convolution's autograd node directly instead of using a full
        # module backward hook, which would fire only at its output boundary.
        self.patch_embed._perf_backward_via_autograd_node = True
...
def _build_mobilenet_v3_small(num_classes: int, input_size: Tuple[int, int]) -> nn.Module:
    """Build a randomly initialized torchvision MobileNetV3-Small."""
    try:
        from torchvision.models import mobilenet_v3_small
    except ImportError as exc:
        raise RuntimeError(
            "The mobilenet_v3_small model requires torchvision to be installed."
        ) from exc

    model = mobilenet_v3_small(weights=None)
    model.classifier[-1] = nn.Linear(model.classifier[-1].in_features, num_classes)
    model.model_metadata = {
        "model_depth": "mobilenet_v3_small",
        "model_width_multiplier": 1.0,
        "model_target_pam_mb": "",
        "model_estimated_pam_mb": "",
        "model_parameter_count": count_parameters(model),
...
def get_model(
    ...
    *,
    ...
    if model_name.lower().replace("-", "_") in {
        "mobilenet_v3_small",
        "mobilenetv3_small",
    }:
        return _build_mobilenet_v3_small(num_classes=num_classes, input_size=input_size)
