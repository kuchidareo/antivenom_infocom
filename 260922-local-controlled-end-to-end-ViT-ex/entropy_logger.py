"""Operator entropy summaries and optional legacy MaxPool Markov measurements.
...
large per-batch trace. ViT summaries cover GELU sign structure and attention
Softmax concentration without copying tensors during the measured operation.
"""
...
class LayerEntropyLogger:
    """Collect online epoch summaries for supported CNN and ViT operators."""
    ...
    def end_batch(self) -> None:
        ...
        for key in sorted(self._forward):
            ...
            if not self.maxpool_markov_only:
                ...
                if isinstance(record.module, torch.nn.ReLU):
                ...
                elif isinstance(record.module, torch.nn.GELU):
                    positive_mask = record.input_tensor.gt(0)
                    self._update(
                        record,
                        family,
                        "forward",
                        "input_positive_rate",
                        float(positive_mask.double().mean().item()),
                    )
                    self._update(
                        record,
                        family,
                        "forward",
                        "input_sign_conditional_entropy_bits",
                        binary_mask_conditional_entropy(positive_mask),
                    )
                elif isinstance(record.module, torch.nn.Softmax):
                    probabilities = record.output_tensor.clamp_min(
                        torch.finfo(record.output_tensor.dtype).tiny
                    )
                    ...
                    self._update(
                        ...
                        float((entropy / math.log2(probabilities.shape[-1])).mean().item()),
                    )
                    self._update(
                        record,
                        family,
