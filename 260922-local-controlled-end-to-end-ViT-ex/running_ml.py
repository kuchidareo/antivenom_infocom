...
def run_one_local(args: argparse.Namespace, poisoning_method: str) -> str:
    ...
    if args.experiment_mode == "frozen_replay":
        checkpoint_payload = load_checkpoint(
            ...
            expected_model_metadata=getattr(model, "model_metadata", {}),
        )
    ...
    entropy_logger = None
    if args.entropy_logging or args.maxpool_markov_only:
    ...
    with LayerPerfLogger(
        ...
        print(
            f"operator_perf={layer_perf_path} operators={len(layer_perf_logger.operator_modules)} "
            f"rows_per_batch={layer_perf_logger.expected_rows_per_batch} "
            f"entropy_summary={entropy_path if entropy_logger is not None else 'disabled'} "
...
def load_checkpoint(
    *,
    ...
    checkpoint_metadata = payload.get("model_metadata", {})
    architecture_keys = (
        "model_depth",
        "patch_size",
        "embed_dim",
    ...
    mismatches = {
        key: (checkpoint_metadata.get(key), expected_model_metadata.get(key))
        for key in architecture_keys
        if checkpoint_metadata.get(key) != expected_model_metadata.get(key)
    }
    if mismatches:
...
def main() -> None:
    ...
    parser.add_argument(
        "--entropy-logging",
        action="store_true",
        help="Calculate operator entropy summaries outside PMU regions (uses extra memory).",
    )
