...
REMOTE_PROJECT_DIR="${REMOTE_PROJECT_DIR:-${REMOTE_REPO_DIR}/260922-local-controlled-end-to-end-ViT-ex}"
...
MODEL_NAME="${MODEL_NAME:-tiny_vit}"
VIT_DEPTH="${VIT_DEPTH:-4}"
VIT_PATCH_SIZE="${VIT_PATCH_SIZE:-4}"
VIT_EMBED_DIM="${VIT_EMBED_DIM:-128}"
VIT_HEADS="${VIT_HEADS:-4}"
VIT_MLP_RATIO="${VIT_MLP_RATIO:-2}"
...
shortcut, and BadSampler condition, the script first trains TinyViT and saves
...
Operator-level forward/backward PMU rows and metrics are written. The measured
operators include QKV, QK matmul, scaling, Softmax, AV matmul, projection,
residual additions, LayerNorm, MLP Linear/GELU, patch projection, and the head.
Remote logs and checkpoints are rsynced into collected_logs/<timestamp>/<target>/.
...
run_command() {
  ...
  local -a command=(
    ...
    --experiment-id "vit_operator_e2e_${RUN_TIMESTAMP}_${target}_${PERF_PRESET}_${scenario}_${experiment_mode}"
    --augment "$augment"
    --perf-events "$PERF_EVENTS"
...
print "Model: ${MODEL_NAME} depth=${VIT_DEPTH} patch=${VIT_PATCH_SIZE} embed=${VIT_EMBED_DIM} heads=${VIT_HEADS} mlp_ratio=${VIT_MLP_RATIO}"
print "Monitoring: operator boundaries, torch_threads=${TORCH_THREADS}"
if (( TRIALS != 1 )); then
