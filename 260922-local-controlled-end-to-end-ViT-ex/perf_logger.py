"""Operator-boundary Linux perf counters for the controlled end-to-end experiment.

Counters are opened once, then reset/enabled immediately before each leaf
operator and disabled immediately after it. No interval sampling thread
is used. Rows stay in memory until the current batch backward pass completes,
so CSV I/O is outside every measured operator region.
"""
...
class LayerPerfLogger:
    """Measure every leaf operator's complete forward and backward execution."""
    ...
    def __init__(
        ...
        *,
        ...
        self.operator_modules = [
            (name, module) for _, name, module in selected_operator_modules
        ]
        # Retain the old attribute for callers that consume this logger as a library.
        self.leaf_modules = self.operator_modules
        if not self.operator_modules:
        ...
        self._layer_by_module = {
            ...
            for index, name, module in selected_operator_modules
        }
        ...
        self._backward_contexts: Dict[torch.nn.Module, List[Dict[str, Any]]] = {}
    ...
    def expected_rows_per_batch(self) -> int:
        return 2 * len(self.operator_modules)
    ...
    def start(self) -> None:
        ...
        try:
            ...
            for module in self._layer_by_module:
                ...
                if not getattr(module, "_perf_backward_via_autograd_node", False):
    ...
    def flush_batch(self) -> None:
        with self._lock:
            ...
            expected = len(self.operator_modules)
    ...
    def _forward_hook(
        ...
        if getattr(module, "_perf_backward_via_autograd_node", False):
            self._register_node_backward(output, backward_context)
            return
        self._backward_contexts.setdefault(module, []).append(backward_context)

    def _register_node_backward(
        self, output: Any, backward_context: Dict[str, Any]
    ) -> None:
        ...
        def backward_pre_hook(grad_outputs: Tuple[Any, ...]) -> Tuple[Any, ...]:
            context = dict(backward_context)
            context["execution_order_index"] = self._phase_order["backward"]
            self._phase_order["backward"] += 1
            self._begin_region(context)
            return grad_outputs
    ...
    def _backward_pre_hook(
        self, module: torch.nn.Module, grad_outputs: Tuple[Any, ...]
    ) -> None:
        if not self._batch_active:
            return
        contexts = self._backward_contexts.get(module, [])
        if not contexts:
            layer = self._layer_by_module[module]
            raise RuntimeError(f"Missing forward context for backward operator {layer[1]}.")
    ...
    def _end_region(self, context: Dict[str, Any]) -> Dict[str, Any]:
        with self._lock:
            ...
            row: Dict[str, Any] = {
                ...
                "perf_measurement_mode": "operator_boundary",
