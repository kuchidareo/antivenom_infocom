...
def _client_command(args: argparse.Namespace, device: dict, seed: int, poisoned_ids: List[str]) -> str:
    ...
    client_args = [
        ...
        *_arg("--device-type", device.get("device_type", "")),
        *_arg("--source-host", device.get("source_host", device["host"])),
    ...
    return f"cd {shlex.quote(remote_project_dir)} && {quoted}"
...
def _start_clients(args: argparse.Namespace, seed: int, poisoned_ids: List[str]) -> List[subprocess.Popen]:
    ...
    for device in DEVICES[: args.num_clients]:
        ...
        ssh_user = device_ssh_user(device) or args.ssh_user
        target = f"{ssh_user}@{device['host']}" if ssh_user else device["host"]
