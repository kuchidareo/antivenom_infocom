...
PLOT_DIR_BY_PHASE = {
    "forward": "plot_forward",
    "backward": "plot_backward",
    "optimizer_step": "plot_optimizer",
}
...
def _dataset_slug(value: Any) -> str:
    text = str(value or "unknown_dataset").strip()
    chars = [ch.lower() if ch.isalnum() else "_" for ch in text]
    slug = "_".join(part for part in "".join(chars).split("_") if part)
    return slug or "unknown_dataset"


def _analysis_trial_id(raw_trial_id: Any, dataset_name: Any) -> str:
    trial_id = _trial_id(raw_trial_id)
    if trial_id.startswith("reference_"):
        return trial_id
    return f"{_dataset_slug(dataset_name)}_{trial_id}"


def _numeric_trial_id(trial_id: str) -> int:
    prefix_rank = 0 if trial_id.startswith("reference_") else 1
    try:
        if "trial_" in trial_id:
            return prefix_rank * 10**8 + int(trial_id.rsplit("trial_", 1)[1].split("_", 1)[0])
    ...
    for trial_id in sorted(data.keys(), key=_numeric_trial_id):
        targets.append(
            Target(
                ...
                target_dataset=str(data[trial_id].get("dataset", "")),
                target_group="clean",
                poisoning_type="none",
...
def resolve_clean_baseline_trial_ids(data: Dict[str, Dict[str, Any]], value: str) -> List[str]:
    """Resolve clean runs used to fit PCA/z-score benign baseline.

    The baseline must be built from reference_* clean runs only. Clean trial_*
    runs are evaluation targets and should not leak into the benign jitter
    distribution.
    """
    if value in {"", "default", "reference", "references", "reference5", "first5"}:
        reference_ids = [trial_id for trial_id in DEFAULT_CLEAN_BASELINE_TRIAL_IDS if trial_id in data]
        if reference_ids:
            return reference_ids
        legacy_ids = [trial_id for trial_id in LEGACY_CLEAN_BASELINE_TRIAL_IDS if trial_id in data]
        if legacy_ids:
            return legacy_ids
        raise ValueError(
            "No clean baseline trials found. Expected reference_0..reference_4 "
            "or legacy trial_0..trial_4."
        )
    trial_ids = [_trial_id(item.strip()) for item in value.split(",") if item.strip()]
    if not trial_ids:
        raise ValueError("At least one clean baseline trial id is required.")
    missing = [trial_id for trial_id in trial_ids if trial_id not in data]
    if missing:
        raise ValueError(f"Clean baseline trial ids do not exist: {missing}")
    return trial_ids
...
def slice_data_for_metric(data: Dict[str, Dict[str, Any]], metric_index: int) -> Dict[str, Dict[str, Any]]:
    ...
    for trial_id, trial_data in data.items():
        ...
        if "dataset" in trial_data:
            sliced_trial["dataset"] = trial_data["dataset"]
...
def compute_all_ot_embeddings(
    ...
    clean_baseline_trial_ids: Sequence[str],
    ...
    missing_baselines = [trial_id for trial_id in clean_baseline_trial_ids if trial_id not in data]
    if missing_baselines:
        raise ValueError(f"Clean baseline trial ids do not exist: {missing_baselines}")
    targets = collect_targets(data, reference_trial_ids[0])
    clean_baseline_trial_set = set(clean_baseline_trial_ids)
                ...
                rows.append(
                    {
                        ...
                        "is_reference_baseline": bool(
                            target.target_group == "clean"
                            and target.target_trial_id in clean_baseline_trial_set
                        ),
...
def fit_clean_pcas(
    ...
    for reference_run_id, cost_type in scopes:
        clean_ids = [
            ...
            and bool(row.get("is_reference_baseline", False))
        ]
        if not clean_ids:
            raise ValueError(f"No clean baseline embeddings found for {reference_run_id}, {cost_type}.")
...
def compute_residual_scores(
    ...
    residual_vectors: Dict[Tuple[str, str, str], np.ndarray] = {}
    for row in summary_rows:
        ...
        residual_vectors[(reference_run_id, cost_type, row["target_run_id"])] = residual.reshape(-1)
...
def add_clean_zscores(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "analysis_phase" in df.columns:
        group_cols.append("analysis_phase")
    for group_key, sub in df.groupby(group_cols, sort=False):
        if isinstance(group_key, tuple):
            ...
            analysis_phase = group_values.get("analysis_phase", "")
        else:
            metric_name = ""
            cost_type = group_key
            segment_type = ""
        ...
        clean = sub[
            ...
            & (sub["is_reference_baseline"].astype(bool))
        ]
        if clean.empty:
            raise ValueError(
                f"No clean reference-baseline rows for z-score group: "
                f"metric={metric_name}, cost={cost_type}, segment={segment_type}:{segment_id}, "
                f"phase={analysis_phase}"
            )
        stats: Dict[str, Any] = {
...
def build_residual_vectors_dataframe(
    summary_rows: Sequence[Dict[str, Any]],
    residual_vectors: Dict[Tuple[str, str, str], np.ndarray],
) -> pd.DataFrame:
    if not residual_vectors:
        return pd.DataFrame()
    max_dim = max(len(vector) for vector in residual_vectors.values())
    meta_cols = [
        "reference_run_id",
        "reference_trial_id",
        "target_trial_id",
    ...
    rows = []
    for row in summary_rows:
        key = (row["reference_run_id"], row["cost_type"], row["target_run_id"])
        vector = residual_vectors[key]
        out = {col: row.get(col, "") for col in meta_cols}
        for idx in range(max_dim):
            out[f"residual_{idx}"] = vector[idx] if idx < len(vector) else ""
        rows.append(out)
    return pd.DataFrame(rows)
...
def _plot_dir_name_for_phase(phase: str) -> str:
    return PLOT_DIR_BY_PHASE.get(str(phase), f"plot_{str(phase)}")
...
def _project_residual_vectors_2d(x: np.ndarray) -> Tuple[np.ndarray, str]:
    """Project full residual vectors to 2D for visualization.

    True t-SNE is used when scikit-learn is installed. The PCA fallback still
    uses the full residual vector, but it is a linear projection.
    """
    if x.shape[0] < 2:
        return np.zeros((x.shape[0], 2), dtype=float), "single_point"
    try:
        from sklearn.manifold import TSNE

        perplexity = max(2, min(10, (x.shape[0] - 1) // 3))
        coords = TSNE(
            n_components=2,
        ...
        pca = PCANP(n_components=2).fit(x)
        coords = pca.transform(x)
        if coords.shape[1] == 1:
            coords = np.column_stack([coords[:, 0], np.zeros(coords.shape[0])])
        return coords[:, :2], "pca_fallback"


def make_residual_vector_plots(
    ...
    residual_cols = [col for col in residual_df.columns if col.startswith("residual_")]
    ...
    group_cols = ["cost_type"]
    if "metric_name" in residual_df.columns:
        group_cols.insert(0, "metric_name")

    for group_key, sub in residual_df.groupby(group_cols, sort=False):
        if isinstance(group_key, tuple):
            group_values = dict(zip(group_cols, group_key))
        else:
            group_values = {"cost_type": group_key}
        metric_name = str(group_values.get("metric_name", "all_metrics"))
        cost_type = str(group_values["cost_type"])
        x = sub[residual_cols].apply(pd.to_numeric, errors="coerce").fillna(0.0).to_numpy(dtype=float)
        coords, method = _project_residual_vectors_2d(x)

        fig, ax = plt.subplots(figsize=(7, 6))
        plot_df = sub.reset_index(drop=True).copy()
        plot_df["residual_projection_x"] = coords[:, 0]
        plot_df["residual_projection_y"] = coords[:, 1]
        plot_df["condition_label"] = plot_df.apply(_condition_label, axis=1)
        for condition, cond_df in plot_df.groupby("condition_label", sort=False):
            condition = str(condition)
            ax.scatter(
                cond_df["residual_projection_x"],
                cond_df["residual_projection_y"],
                s=34,
                alpha=0.78,
        ...
        ax.set_title(f"Full residual-vector {method}: {metric_name} / {cost_type}")
        ax.set_xlabel(f"residual {method} x")
        ax.set_ylabel(f"residual {method} y")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=8, loc="best")
        fig.tight_layout()
        safe_metric = metric_name.replace("/", "_")
        safe_cost = cost_type.replace("/", "_")
        fig.savefig(plot_dir / f"residual_tsne_{safe_metric}_{safe_cost}.png", dpi=180)
        plt.close(fig)


def make_plots(df: pd.DataFrame, output_dir: Path, plot_dir_name: str = "plots") -> None:
    os.environ.setdefault("MPLCONFIGDIR", str(output_dir / ".matplotlib_cache"))
    try:
        import matplotlib.pyplot as plt
    except ImportError as exc:
...
def save_outputs(
    ...
    if residual_df is not None and not residual_df.empty:
        residual_df.to_csv(output_dir / "residual_vectors.csv", index=False)
    if "analysis_phase" in zscore_df.columns:
        for phase, phase_df in zscore_df.groupby("analysis_phase", sort=False):
            make_plots(phase_df, output_dir, _plot_dir_name_for_phase(str(phase)))
            if residual_df is not None and not residual_df.empty:
                phase_residual_df = residual_df[residual_df["analysis_phase"].astype(str) == str(phase)]
                make_residual_vector_plots(phase_residual_df, output_dir, _plot_dir_name_for_phase(str(phase)))
    else:
        make_plots(zscore_df, output_dir)
        if residual_df is not None and not residual_df.empty:
            make_residual_vector_plots(residual_df, output_dir)
...
def _read_hardware_csv_segments(
...
) -> List[Tuple[str, str, str, np.ndarray, Dict[str, str], int]]:
    df = pd.read_csv(path)
    if df.empty:
        raise ValueError(f"{path} is empty.")
    missing = [column for column in feature_columns if column not in df.columns]
    ...
    for segment_type, segment_id, segment_df in groups:
        if "phase" not in segment_df.columns:
            raise ValueError(f"{path} is missing phase column required for phase-specific OT analysis.")
        phase_series = segment_df["phase"].astype(str)
        for phase in TRAINING_PHASES:
            phase_df = segment_df.loc[phase_series == phase].copy()
            if not phase_df.empty:
                phase_groups.append((segment_type, segment_id, phase, phase_df))

    out: List[Tuple[str, str, str, np.ndarray, Dict[str, str], int]] = []
    for segment_type, segment_id, analysis_phase, segment_df in phase_groups:
        ...
        first["analysis_phase"] = analysis_phase
        out.append((segment_type, segment_id, analysis_phase, values, first, original_length))
    return out
...
def load_data_from_csv_paths(
    ...
    for path in csv_paths:
        ...
        dataset_name = str(first.get("dataset", ""))
        trial_id = _analysis_trial_id(first.get("trial_id", ""), dataset_name)
        ...
        data[trial_id]["dataset"] = dataset_name
...
def load_segmented_data_from_csv_paths(
    ...
    segment_by: str,
    max_samples_per_run: int = 0,
) -> Tuple[Dict[Tuple[str, str, str], Dict[str, Dict[str, Any]]], Dict[str, Dict[str, str]]]:
    if not csv_paths:
        raise ValueError("No hardware CSV files found.")

    segmented_data: Dict[Tuple[str, str, str], Dict[str, Dict[str, Any]]] = {}
    metadata: Dict[str, Dict[str, str]] = {}
    seen: Dict[Tuple[str, str, str, str, str], Path] = {}

    for path in csv_paths:
        for segment_type, segment_id, analysis_phase, run, first, original_length in _read_hardware_csv_segments(
            path=path,
            ...
            trial_id = _analysis_trial_id(first.get("trial_id", ""), dataset_name)
            poisoning_type = str(first.get("poisoning_method", "") or "clean")
            segment_key = (segment_type, segment_id, analysis_phase)
            seen_key = (segment_type, segment_id, analysis_phase, trial_id, poisoning_type)
            if seen_key in seen:
                raise ValueError(
                    "Duplicate segment for "
                    f"segment={segment_key} phase={analysis_phase} trial={trial_id} "
                    f"poisoning_method={poisoning_type}: "
            ...
            if poisoning_type == "clean":
                ...
                data[trial_id]["poisoning"][poisoning_type] = run
                run_id = f"{trial_id}_{poisoning_type}"
            metadata[f"{segment_type}_{segment_id}_{analysis_phase}_{run_id}"] = {
                **first,
                "source_path": str(path),
                "original_length": str(original_length),
                "analysis_length": str(run.shape[0]),
...
def run_one_group(
    *,
    ...
    for segment_type, segment_id, analysis_phase in sorted(
        segmented_data.keys(),
        key=lambda item: (item[0], int(item[1]) if item[1].isdigit() else item[1], item[2]),
    ):
        print(f"segment_type={segment_type} segment_id={segment_id} phase={analysis_phase}")
        segment_data = segmented_data[(segment_type, segment_id, analysis_phase)]
        for metric_index, source_column in enumerate(feature_columns):
            ...
            clean_baseline_trial_ids = resolve_clean_baseline_trial_ids(data, clean_baseline_trial_ids_value)
            print(
            ...
            print(f"Using clean baseline trials: {','.join(clean_baseline_trial_ids)}")
            summary_rows, embeddings = compute_all_ot_embeddings(
                data=data,
                reference_trial_ids=reference_trial_ids,
                clean_baseline_trial_ids=clean_baseline_trial_ids,
            ...
            residual_vectors = compute_residual_scores(summary_rows, embeddings, pcas)
            for row in summary_rows:
                row["segment_type"] = segment_type
                row["segment_id"] = segment_id
                row["analysis_phase"] = analysis_phase
                row["metric_name"] = metric_name
                row["source_column"] = source_column
                row["metric_transform"] = metric_transform
            residual_frames.append(build_residual_vectors_dataframe(summary_rows, residual_vectors))
    ...
    save_outputs(output_dir, summary_df, zscore_df, baseline_df, residual_df)
...
def main() -> None:
    ...
    parser.add_argument(
        "--clean_baseline_trial_ids",
        default="default",
        help="Clean runs used to fit PCA/z-score benign baseline. Default uses reference_0..reference_4.",
    )
    ...
    for group_label, group_dir in groups:
        ...
        run_one_group(
            ...
            clean_baseline_trial_ids_value=args.clean_baseline_trial_ids,
