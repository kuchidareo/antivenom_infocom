...
def _plot_condition(row: pd.Series) -> str:
    return "clean" if row["target_group"] == "clean" else str(row["poisoning_type"])


def _plot_marker(condition: str) -> str:
    return "o" if condition == "clean" else "s"


def _plot_color_for_condition(condition: str) -> str:
    palette = {
        "clean": "tab:blue",
        "unlearnable_examples": "tab:red",
        "random_label_flipping": "tab:green",
    ...
    return palette.get(condition, "tab:gray")


def _segment_axis_label(df: pd.DataFrame) -> str:
    if "segment_type" not in df.columns:
        return "round"
    segment_types = {str(value) for value in df["segment_type"].dropna().unique()}
    if segment_types == {"epoch"}:
        return "epoch"
    if segment_types == {"round"}:
        return "round"
    if segment_types == {"full_run"}:
        return "run"
    return "round/epoch"


def _safe_plot_name(value: Any) -> str:
    text = str(value)
    return "".join(char if char.isalnum() or char in "._-" else "_" for char in text).strip("_")


def make_ot_distance_plots(df: pd.DataFrame, plot_dir: Path) -> None:
    """Plot raw OT distance over the analysis segment.

    Rows are cost functions. The x-axis is the segment used by the OT
    calculation: FL round for FL logs, or local epoch for local-ML logs.
    Multiple trials for the same condition/segment are summarized as mean
    +/- one standard deviation.
    """
    required = {"cost_type", "ot_cost", "target_group", "poisoning_type"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"Cannot make OT distance plots; missing columns: {sorted(missing)}")

    working = df.copy()
    if "segment_id" in working.columns:
        working["segment_index"] = pd.to_numeric(working["segment_id"], errors="coerce")
    else:
        working["segment_index"] = pd.to_numeric(
            working.get("target_trial_id", pd.Series(index=working.index, dtype=object))
            .astype(str)
            .str.extract(r"(\d+)$", expand=False),
            errors="coerce",
        )
    working = working[working["segment_index"].notna()].copy()
    if working.empty:
        return

    working["segment_index"] = working["segment_index"].astype(int)
    working["condition"] = working.apply(_plot_condition, axis=1)
    if "metric_name" not in working.columns:
        working["metric_name"] = "all_metrics"

    x_label = _segment_axis_label(working)
    cost_types = [cost_type for cost_type in COST_TYPES if cost_type in set(working["cost_type"])]
    cost_types.extend(
        cost_type for cost_type in working["cost_type"].dropna().unique() if cost_type not in cost_types
    )
    ...
    for metric_name, metric_df in working.groupby("metric_name", sort=False):
        fig, axes = plt.subplots(
            len(cost_types),
            1,
            figsize=(8.5, max(2.8 * len(cost_types), 3.0)),
            squeeze=False,
            sharex=True,
        )
        for ax, cost_type in zip(axes[:, 0], cost_types):
            sub = metric_df[metric_df["cost_type"] == cost_type].copy()
            if sub.empty:
                ax.set_visible(False)
                continue
            grouped = (
                sub.groupby(["condition", "segment_index"], as_index=False)["ot_cost"]
                .agg(["mean", "std", "count"])
                .reset_index()
            )
            for condition, cdf in grouped.groupby("condition", sort=False):
                cdf = cdf.sort_values("segment_index")
                color = _plot_color_for_condition(condition)
                ax.plot(
                    cdf["segment_index"],
                    cdf["mean"],
                    marker=_plot_marker(condition),
                    color=color,
                    linewidth=1.5,
                    label=condition,
                )
        ...
        fig.tight_layout()
        filename = f"ot_distance_by_{_safe_plot_name(x_label)}_{_safe_plot_name(metric_name)}.png"
...
def make_plots(df: pd.DataFrame, output_dir: Path, plot_dir_name: str = "plots") -> None:
    ...
    make_ot_distance_plots(df, plot_dir)

    plot_specs = [
        ("pca_x", "pca_y", "pca_scatter_{metric_name}_{cost_type}.png", "PCA x", "PCA y"),
        (
            "ot_cost",
            "residual_norm",
            "ot_vs_residual_{metric_name}_{cost_type}.png",
            "OT cost",
            "Residual norm",
        ),
        (
            "z_ot_cost",
            "z_residual_norm",
            "z_ot_vs_z_residual_{metric_name}_{cost_type}.png",
            "Z OT cost",
            "Z residual norm",
        ),
        (
            "z_ot_cost",
            "z_residual_ratio",
            "z_ot_vs_z_residual_ratio_{metric_name}_{cost_type}.png",
            "Z OT cost",
            "Z residual ratio",
        ),
    ]
        ...
        for x_col, y_col, filename_template, xlabel, ylabel in plot_specs:
            ...
            fig.savefig(
                ...
                / filename_template.format(
                    metric_name=_safe_plot_name(metric_name),
                    cost_type=_safe_plot_name(cost_type),
                ),
                dpi=180,
            )
            plt.close(fig)
