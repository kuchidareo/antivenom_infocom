...
from experiment_config import (
    ...
    DEFAULT_BATCH_SIZE,
    DEFAULT_DATA_DIR,
    DEFAULT_NONIID_ALPHA,
    DEFAULT_NUM_CLIENTS,
    DEFAULT_PARTITION_METHOD,
...
def _resolve_metadata_image_path(
    ...
    When data_dir is now ../non-iid-data, the correct path is
    ../non-iid-data/small_trashnet/clean/...
    """
...
def _assign_dirichlet_noniid_partitions(
    ...
    """Assign label-skew non-IID partitions using a Dirichlet distribution.

    For each class, client proportions are sampled from Dirichlet(alpha). Lower
    alpha gives stronger label skew. A final rebalance step guarantees every
    client receives at least one sample, which avoids empty local datasets.
    """
    import numpy as np
    from collections import defaultdict
    ...
    empty_clients = [client_idx for client_idx, values in client_indices.items() if not values]
    for empty_client in empty_clients:
        ...
        moved_idx = client_indices[donor].pop()
        client_indices[empty_client].append(moved_idx)

    assignment: Dict[int, str] = {}
    for client_idx, indices in client_indices.items():
        for item_idx in indices:
            assignment[item_idx] = f"client_{client_idx}"
    return assignment
...
def _assign_partitions(
    ...
    *,
    ...
    raise ValueError(
        "Unknown partition_method="
        f"{partition_method!r}. Use 'iid' or 'dirichlet_noniid'."
    )
