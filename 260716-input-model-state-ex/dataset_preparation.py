...
def _add_clean_test_split(
    ...
    *,
    ...
    for label in sorted(by_label):
        candidates = sorted(set(by_label[label]), key=int)
        rng.shuffle(candidates)
        count = max(1, int(round(len(candidates) * test_fraction)))
        count = min(count, len(candidates) - 1)
        if count <= 0:
            raise ValueError(f"Class {label} has too few samples for a test split.")
        test_source_indices.update(candidates[:count])
...
def prepare_dataset(
    *,
    ...
    clean_records = _clean_records_from_metadata(metadata_rows)
    training_clean_records = [
