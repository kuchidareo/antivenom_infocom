...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model_variant",
        default="simple",
        choices=["simple", "pam500mb"],
        help="Model-size log group to plot.",
    )
    parser.add_argument(
        "--epoch",
        type=int,
...
def infer_model_variant(path: Path, row: Dict[str, str]) -> str:
    if "pam500mb" in path.parts:
        return "pam500mb"
    if "simple" in path.parts:
        return "simple"
    if row.get("model") == "pam_cnn":
        return "pam500mb"
    return "simple"


def discover_runs(input_dir: Path, device: str, trial_id: str, model_variant: str) -> List[RunInfo]:
    device_dir = input_dir / "model_size_quick" / device / model_variant
    if device_dir.exists():
        candidate_paths = sorted(device_dir.glob("*.csv"))
    else:
        print(f"warning: expected model-size directory does not exist: {device_dir}; searching recursively")
        candidate_paths = sorted(input_dir.glob("**/*.csv"))
    ...
    for path in candidate_paths:
        ...
        if first.get("trial_id") != trial_id:
            continue
        if (first.get("device_id") or first.get("host") or device) != device:
            continue
        inferred_variant = infer_model_variant(path, first)
        if inferred_variant != model_variant:
            continue
        runs.append(
            RunInfo(
                path=path,
                ...
                model_variant=inferred_variant,
                model_target_pam_mb=first.get("model_target_pam_mb") or "",
...
def plot_combined(
    ...
    fig.suptitle(f"{device} {model_variant} local_ml {trial_id} {epoch_label} ({cpu_label})", y=0.995)
    fig.tight_layout(rect=[0, 0, 1, 0.985])

    output_path = (
        output_dir
        / f"raw_timeseries_{safe_name(device)}_{safe_name(model_variant)}_{safe_name(trial_id)}_{epoch_label}_{cpu_label}.png"
    )
...
def plot_individual(
    ...
    for column in PLOT_COLUMNS:
        ...
        ax.set_title(f"{device} {model_variant} {trial_id} {epoch_label} / {DISPLAY_NAMES[column]}")
        ...
        output_path = (
            output_dir
            / (
                f"raw_{safe_name(DISPLAY_NAMES[column])}_{safe_name(device)}_"
                f"{safe_name(model_variant)}_{safe_name(trial_id)}_{epoch_label}_{cpu_label}.png"
            )
        )
        fig.savefig(output_path, dpi=180)
        fig.savefig(output_path.with_suffix(".pdf"))
...
def write_selection_summary(
        ...
        rows.append(
            {
                ...
                "model": run.model,
                "model_target_pam_mb": run.model_target_pam_mb,
                "trial_id": trial_id,
    ...
    output_path = (
        output_dir
        / f"raw_timeseries_selection_{safe_name(device)}_{safe_name(model_variant)}_{safe_name(trial_id)}.csv"
    )
