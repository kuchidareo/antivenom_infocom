...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--batch-time-epochs",
        "--batch-time-epoch",
        dest="batch_time_epochs",
        type=int,
        nargs="+",
        default=[0, 10],
        help="Global epochs shown in batch-time figures (default: 0 10).",
    )
    parser.add_argument(
        "--bin-ms",
        type=float,
    ...
    parser.add_argument(
        "--performance-only",
        action="store_true",
        help="Only generate the per-epoch training and clean-evaluation performance figure.",
    )
...
def load_sequence_runs(input_dir: Path) -> dict[str, dict[str, object]]:
    ...
    for sequence, (main_path, main_frame) in sorted(candidates.items()):
        perf_path = main_path.with_name(f"{main_path.stem}_perf.csv")
        metrics_path = main_path.with_name(f"{main_path.stem}_metrics.csv")
        if not perf_path.exists():
            raise FileNotFoundError(f"Missing perf CSV: {perf_path}")
...
def plot_sequence_metric(
    ...
    epoch_context = ""
    if x_mode != "epoch":
        context_frame = run["main"]
        assert isinstance(context_frame, pd.DataFrame)
        method_column = (
            "input_poisoning_method"
            if "input_poisoning_method" in context_frame.columns
            else "poisoning_method"
        )
        epoch_rows = context_frame.loc[
            pd.to_numeric(context_frame["epoch"], errors="coerce").eq(batch_time_epoch)
        ]
        methods = epoch_rows[method_column].dropna().unique()
        method_label = METHOD_LABELS.get(str(methods[0]), str(methods[0])) if len(methods) else "N/A"
        epoch_context = f" | Epoch {batch_time_epoch} ({method_label})"
    axis.set_title(
        f"{label} | {sequence_label(sequence)}{epoch_context} | {device}",
        fontsize=8,
        pad=3,
    )
    if x_mode == "epoch":
...
def save_sequence_phase_figure(
    ...
    normalize_per_instruction: bool = False,
    x_mode: str = "epoch",
    batch_time_epochs: tuple[int, ...] = (0, 10),
    bin_sec: float = 0.1,
) -> Path:
    metrics = PERF_METRICS + ELAPSED_METRICS if normalize_per_instruction else METRICS
    sequence_order = {
        "clean_to_availability_shortcuts": 0,
        "availability_shortcuts_to_clean": 1,
    }
    sequences = sorted(runs, key=lambda value: (sequence_order.get(value, 99), value))
    panels = (
        [(sequence, batch_time_epochs[0]) for sequence in sequences]
        if x_mode == "epoch"
        else [
            (sequence, epoch)
            for sequence in sequences
            for epoch in batch_time_epochs
        ]
    )
    figure, axes = plt.subplots(
        nrows=len(metrics),
        ncols=len(panels),
        figsize=(8 * len(panels), len(metrics) * 2.15),
        sharex=False,
        sharey=False,
        squeeze=False,
    )
    for row, metric in enumerate(metrics):
        plotted_axes = []
        for column_index, (sequence, batch_time_epoch) in enumerate(panels):
            plotted = plot_sequence_metric(
            ...
            if plotted:
                plotted_axes.append(axes[row, column_index])
        if plotted_axes:
            y_min = min(axis.get_ylim()[0] for axis in plotted_axes)
            y_max = max(axis.get_ylim()[1] for axis in plotted_axes)
    ...
    comparison = (
        ...
        if x_mode == "epoch"
        else f"Global Epochs {', '.join(map(str, batch_time_epochs))} Batch-Time Mean and Standard Deviation"
    )
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    prefix = "metrics_per_instruction" if normalize_per_instruction else "raw_metrics"
    epoch_suffix = "_".join(map(str, batch_time_epochs))
    suffix = "" if x_mode == "epoch" else f"_epochs{epoch_suffix}_batch_time"
    output_path = output_dir / f"{prefix}_{phase}{suffix}.png"
    figure.savefig(output_path, dpi=dpi, bbox_inches="tight", facecolor="white")
    plt.close(figure)
    return output_path
...
def performance_rows(
    ...
    required = {"epoch", "metric_event", value_column}
    if not required.issubset(metrics.columns):
        return pd.DataFrame()
    selected = metrics.loc[
        metrics["metric_event"].isin(event_names),
        ["epoch", value_column],
    ].copy()
    selected["epoch"] = pd.to_numeric(selected["epoch"], errors="coerce")
    selected["value"] = pd.to_numeric(selected[value_column], errors="coerce")
    return (
        selected.dropna(subset=["epoch", "value"])
        .groupby("epoch", as_index=False)["value"]
        .last()
        .sort_values("epoch")
    )


def save_sequence_performance_figure(
    runs: dict[str, dict[str, object]],
    output_dir: Path,
    dpi: int,
) -> Path:
    ...
    sequences = sorted(runs, key=lambda value: (sequence_order.get(value, 99), value))
    specs = (
        ("Training accuracy (%)", ("train_epoch",), "accuracy", 100.0),
        (
            "Clean test accuracy (%)",
            ("clean_test_epoch", "clean_evaluation_epoch", "clean_holdout_epoch"),
            "accuracy",
            100.0,
        ),
        ("Training loss", ("train_epoch",), "loss", 1.0),
        (
            "Clean test loss",
            ("clean_test_epoch", "clean_evaluation_epoch", "clean_holdout_epoch"),
            "loss",
    ...
    figure, axes = plt.subplots(
        nrows=len(specs),
        ncols=len(sequences),
        figsize=(9 * len(sequences), 10),
        sharex=True,
        sharey=False,
        squeeze=False,
    )

    for row, (label, events, value_column, scale) in enumerate(specs):
        plotted_axes = []
        for column, sequence in enumerate(sequences):
            axis = axes[row, column]
            metrics = runs[sequence]["metrics"]
            assert isinstance(metrics, pd.DataFrame)
            summary = performance_rows(metrics, events, value_column)
            train_epochs = metrics.loc[
                metrics["metric_event"].eq("train_epoch"),
                ["epoch", "input_poisoning_method"],
            ].copy()
            train_epochs["epoch"] = pd.to_numeric(train_epochs["epoch"], errors="coerce")
            method_by_epoch = train_epochs.dropna(subset=["epoch"]).drop_duplicates("epoch").set_index(
                "epoch"
            )["input_poisoning_method"]
            if not summary.empty:
                summary["value"] *= scale
                summary["method"] = summary["epoch"].map(method_by_epoch)
                for method in SEQUENCE_METHODS:
                    segment = summary.loc[summary["method"].eq(method)]
                    if segment.empty:
                        continue
                    axis.plot(
                        segment["epoch"],
            ...
            axis.axvline(9.5, color="#777777", linestyle="--", linewidth=0.8, alpha=0.8)
            axis.set_title(f"{label} | {sequence_label(sequence)}", fontsize=9, pad=4)
            axis.set_xlim(-0.4, 19.4)
            axis.xaxis.set_major_locator(MaxNLocator(integer=True))
            axis.set_xlabel("Global epoch", fontsize=8)
    ...
    handles.append(
        Line2D([0], [0], color="#777777", linestyle="--", linewidth=0.8, label="Stage boundary")
    )
    figure.suptitle("Sequence Model Performance by Epoch", fontsize=15, y=0.995)
    figure.legend(
        handles=handles,
        loc="upper center",
        ncol=len(handles),
    ...
    figure.tight_layout(rect=(0.015, 0.01, 0.995, 0.925), h_pad=1.2, w_pad=1.2)

    output_dir.mkdir(parents=True, exist_ok=True)
    output_path = output_dir / "sequence_model_performance.png"
    figure.savefig(output_path, dpi=dpi, bbox_inches="tight", facecolor="white")
    plt.close(figure)
    return output_path
...
def main() -> None:
    ...
    if not args.batch_time_epochs or any(epoch < 0 for epoch in args.batch_time_epochs) or args.bin_ms < 0:
        raise ValueError("--batch-time-epochs and --bin-ms must be non-negative")
    ...
    if args.performance_only:
        if not sequence_mode:
            raise ValueError("--performance-only requires sequence logs")
        path = save_sequence_performance_figure(runs, args.output_dir.resolve(), args.dpi)
        print(f"Saved {path}")
        return
    for phase in ("forward", "backward"):
        for normalize_per_instruction in (False, True):
            for x_mode in ("epoch", "batch_time"):
                if sequence_mode:
                    path = save_sequence_phase_figure(
                        runs,
                        phase,
