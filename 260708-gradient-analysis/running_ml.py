...
from optimizer_input_logger import GradientStreamLogger
...
def run_one_local(args: argparse.Namespace, poisoning_method: str) -> str:
    ...
    with HardwareLogger(log_dir=args.log_dir, condition=condition, training_state=state) as logger:
        ...
        gradient_stream_logger = None
        if not args.disable_optimizer_input_inspection:
            gradient_stream_logger = GradientStreamLogger(
                path=logger.path.with_name(f"{logger.path.stem}_gradient_streams.csv"),
                condition=condition,
            )
        train_model(
            model=model,
            ...
            gradient_stream_logger=gradient_stream_logger,
        )
        evaluate_model(
            model=model,
...
def main() -> None:
    ...
    parser.add_argument(
        "--disable-optimizer-input-inspection",
        action="store_true",
        help="Disable per-parameter gradient stream CSV logging.",
    )
