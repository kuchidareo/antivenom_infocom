...
def resolve_torch_device(requested: str = "auto") -> torch.device:
    ...
    if requested == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA was requested with --torch-device cuda, but torch.cuda.is_available() is False.")
    return torch.device(requested)


def cuda_metadata(device: Optional[torch.device] = None) -> Dict[str, Any]:
