...
from training_utils import cuda_metadata, evaluate_model, get_parameters, resolve_torch_device, set_parameters, train_model
...
class TrashNetFlowerClient(fl.client.NumPyClient):
    def __init__(self, args: argparse.Namespace, state: TrainingState) -> None:
        ...
        self.torch_device = resolve_torch_device(args.torch_device)
        self.args.torch_device_actual = str(self.torch_device)
        for key, value in cuda_metadata(self.torch_device).items():
...
def main() -> None:
    ...
    args.torch_device_actual = str(torch_device)
    for key, value in cuda_metadata(torch_device).items():
